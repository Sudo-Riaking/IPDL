...
import {
  Newspaper, Mail, Phone, Download, Quote, ExternalLink, Mic, Calendar, ArrowRight,
} from "lucide-react";
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="border-b border-slate-900 pb-8 mb-10">
          <span className="text-[10px] mono-text uppercase tracking-widest text-slate-500 font-bold mb-2 flex items-center gap-2">
            <Newspaper className="h-4 w-4 text-rose-400" /> Espace presse
          </span>
          <h1 className="text-3xl font-extrabold text-white sm:text-4xl">Presse &amp; Médias</h1>
          <p className="mt-2 text-slate-400 text-sm max-w-2xl">
            Journalistes, vous préparez un reportage sur la modélisation des systèmes complexes, l&apos;IA, le climat ou la santé ? Retrouvez ici nos contacts, nos expert·e·s à interviewer et nos temps forts.
          </p>
        ...
        <section className="grid gap-6 lg:grid-cols-3 mb-14">
          <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-900/60 to-slate-900/10 p-8">
            <h2 className="text-lg font-bold text-white mb-2">Contact presse</h2>
            <p className="text-sm text-slate-400 leading-relaxed mb-5 max-w-2xl">
              Notre service communication répond aux demandes d&apos;interviews, met en relation avec les chercheur·euse·s et fournit visuels et éléments de langage.
            </p>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/10 p-8 flex flex-col">
            <h2 className="text-lg font-bold text-white mb-2">Kit presse</h2>
            <p className="text-sm text-slate-400 leading-relaxed flex-1">
              Logos officiels, chiffres clés (94 membres · 5 centres · 4 axes) et présentation de l&apos;unité.
            </p>
            <a href="/logos/logo_ummisco.png" download className="mt-4 inline-flex items-center justify-center gap-2 rounded-lg border border-slate-700 px-4 py-2.5 text-sm font-semibold text-slate-200 hover:border-slate-600 transition-all">
              <Download className="h-4 w-4" /> Télécharger le logo
            </a>
          </div>
        </section>
        ...
        <section className="mb-14">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="text-sm font-bold text-white uppercase tracking-wider">Nos expert·e·s à interviewer</h2>
          </div>
              ...
              <div key={e.id} className="rounded-xl border border-slate-900 bg-slate-950 p-5 flex flex-col">
                ...
                <div className="mt-auto flex items-center gap-3 pt-3 border-t border-slate-900/60">
                  ...
                  <a href={scholarUrl({ title: e.name })} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[12px] text-slate-400 hover:text-slate-200 transition-colors">
                    <Quote className="h-3 w-3" /> Travaux
                  </a>
                </div>
              </div>
            ))}
        ...
        <section className="grid gap-10 lg:grid-cols-2">
          <div>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2"><Quote className="h-4 w-4 text-green-400" /> Publications à la une</h2>
          ...
          <div>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2"><Calendar className="h-4 w-4 text-amber-400" /> Prochains rendez-vous</h2>
              <Link href="/actualites" className="text-[12px] font-semibold text-blue-400 hover:text-blue-300 inline-flex items-center gap-1">Tout l&apos;agenda <ArrowRight className="h-3 w-3" /></Link>
            </div>
            <div className="space-y-3">
              {upcoming.map((s) => (
              ...
              {upcoming.length === 0 && (<p className="text-sm text-slate-500 italic">Aucun rendez-vous programmé pour le moment.</p>)}
            </div>
          </div>
        </section>
      </main>
