        ...
        <section className="relative flex flex-col justify-center min-h-[70vh] py-20 px-4 sm:px-6 lg:px-8 overflow-hidden bg-radial from-slate-900/80 via-slate-950 to-slate-950 border-b border-slate-900">
          ...
          <div className="mx-auto max-w-4xl text-center relative z-10 flex flex-col items-center">
            <motion.div
              ...
              className="inline-flex items-center gap-1.5 rounded-full border border-slate-800 bg-slate-900/60 px-3.5 py-1 text-sm text-slate-400 mb-6"
            >
            ...
            <motion.p
              ...
              className="mt-6 max-w-2xl text-slate-400 text-base sm:text-base leading-relaxed"
            >
              {t("hero.description")}
            </motion.p>
              ...
              {searchQuery && (
                <div className="absolute top-full left-0 right-0 mt-3 rounded-2xl border border-slate-800 bg-slate-900/95 backdrop-blur-md shadow-2xl p-6 text-left z-30 max-h-96 overflow-y-auto">
                  <div className="flex items-center justify-between pb-3 border-b border-slate-800 mb-4">
                    <span className="text-[13px] mono-text uppercase tracking-wider text-slate-500 font-bold">
                  ...
                  <div className="space-y-6">
                    ...
                    {filteredResearchers.length > 0 && (
                      <div>
                        <h4 className="text-[13px] font-bold text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                        ...
                        <div className="space-y-2">
                          {filteredResearchers.map((r) => (
                            ...
                            >
                              ...
                              <div className="text-[13px] text-slate-500">{r.title}</div>
                            </Link>
                          ))}
                        </div>
                      </div>
                        ...
                        <div className="space-y-2">
                          {filteredPubs.map((p) => (
                            <Link
                              ...
                              className="block p-2 rounded-lg hover:bg-slate-800/60 transition-colors"
                            >
                              <div className="text-sm font-bold text-slate-200 line-clamp-1">{p.title}</div>
                              <div className="text-[13px] text-slate-500">{p.authors.join(", ")} ({p.year})</div>
                            </Link>
                          ))}
                        </div>
                      </div>
                        ...
                        <div className="space-y-2">
                          {filteredDatasets.map((d) => (
                            <a
                              ...
                              className="block p-2 rounded-lg hover:bg-slate-800/60 transition-colors"
                            >
                              <div className="text-sm font-bold text-slate-200">{d.title}</div>
                              <div className="text-[13px] text-slate-500">{d.size} · {d.accessLevel}</div>
                            </a>
                          ))}
                        </div>
                      </div>
                  ...
                  <div className="space-y-4">
                    {PUBLICATIONS.filter((p) => p.axis === selectedAxis).map((p) => (
                      <div key={p.id} className="border-b border-slate-900/60 pb-3 last:border-b-0 last:pb-0">
                        ...
                        <div className="text-[13px] text-slate-400 mt-1">{p.authors.join(", ")} — {p.year}</div>
                      </div>
                    ))}
                    {PUBLICATIONS.filter((p) => p.axis === selectedAxis).length === 0 && (
                      <div className="text-sm text-slate-500">Aucune publication indexée sur cet axe pour le moment.</div>
                    )}
                  </div>
                </motion.div>
              )}
                ...
                >
                  <div>
                    ...
                    <div className="flex justify-between items-center mb-4">
                      <span className="inline-flex items-center rounded-full bg-slate-900 px-2 py-0.5 text-[12px] font-semibold text-slate-400 border border-slate-800 uppercase tracking-wider">
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-900/60 flex items-center justify-between">
                    <span className="text-[13px] text-slate-500 italic truncate max-w-[60%]">
                      {pub.authors.join(", ")}
                    </span>
                    <button
                      onClick={() => setCitationModalPub(pub)}
                      className="inline-flex items-center gap-1.5 rounded bg-blue-600/10 px-2.5 py-1.5 text-[13px] font-bold text-blue-400 border border-blue-900/30 hover:bg-blue-600/20 active:scale-95 transition-all"
                    >
                    ...
                    <div>
                      ...
                      <div className="flex justify-between items-start border-b border-slate-900/60 pb-3 mb-4">
                        ...
                        <div className="flex items-center gap-1 pl-2">
                          {isPublic && (
                            <span className="inline-flex items-center gap-1 rounded bg-green-500/10 px-2 py-0.5 text-[12px] font-bold text-green-400 border border-green-900/30 uppercase tracking-wider">
                              <Eye className="h-2.5 w-2.5" /> Public
                            </span>
                          )}
                          {isProtected && (
                            <span className="inline-flex items-center gap-1 rounded bg-blue-500/10 px-2 py-0.5 text-[12px] font-bold text-blue-400 border border-blue-900/30 uppercase tracking-wider">
                              <Lock className="h-2.5 w-2.5" /> Protégé
                            </span>
                          )}
                          {isPrivate && (
                            <span className="inline-flex items-center gap-1 rounded bg-red-500/10 px-2 py-0.5 text-[12px] font-bold text-red-400 border border-red-900/30 uppercase tracking-wider">
                    ...
                    <div className="mt-4 flex items-center justify-between text-sm text-slate-500">
                      ...
                      {isPublic ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 rounded bg-green-600 px-3 py-1.5 text-[13px] font-bold uppercase tracking-wider text-white hover:bg-green-700 active:scale-95 transition-all"
                        >
                        ...
                        <Link
                          href="/connexion"
                          className="inline-flex items-center gap-1.5 rounded bg-slate-900 border border-slate-800 px-3 py-1.5 text-[13px] font-bold uppercase tracking-wider text-slate-400 hover:text-slate-200"
                        >
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {RESEARCHERS.map((res) => (
                ...
                >
                  ...
                  <p className="mt-1.5 text-[13px] text-slate-500 font-medium leading-normal">
                    {res.title}
                  </p>
                  <div className="mt-4 flex flex-wrap gap-1 justify-center">
                    {res.axes.map((a) => (
                      <span
                        key={a}
                        className="text-[11px] bg-slate-900 border border-slate-800 text-slate-400 px-1.5 py-0.5 rounded uppercase"
                      >
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {SEMINARS.map((sem) => (
                ...
                >
                  <div>
                    <div className="flex items-center gap-2 text-blue-500 mb-4 text-[13px] font-bold tracking-wider uppercase">
                  ...
                  <div className="mt-6 border-t border-slate-900 pt-3">
                    <div className="text-[13px] text-slate-400">{t("events.speaker")} <strong className="text-slate-300 font-semibold">{sem.speaker}</strong></div>
                    <div className="text-[12px] text-slate-500 mt-0.5 truncate">{sem.location}</div>
                  </div>
                </div>
              ))}
            </div>
                ...
                >
                  <div className="space-y-4">
                    <div className="flex justify-between items-start">
                      <h3 className="text-base font-bold text-white leading-snug">{contract.title}</h3>
                      <span className="inline-flex items-center rounded-full bg-blue-600/10 px-2 py-0.5 text-[12px] font-semibold text-blue-400 border border-blue-900/30 whitespace-nowrap">
                    ...
                    <div className="p-3 rounded bg-slate-900/10 dark:bg-slate-900/60 border border-slate-800 dark:border-slate-900 text-[13px] text-slate-400 dark:text-slate-500">
                      <strong className="text-slate-300 dark:text-slate-400 block mb-1">Prérequis :</strong>
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-900/60 flex justify-end">
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600/10 px-4 py-2 text-[13px] font-bold uppercase tracking-wider text-blue-400 border border-blue-900/30 hover:bg-blue-600/20 active:scale-95 transition-all"
                    >
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="grid gap-8 lg:grid-cols-3">
              ...
              <div className="lg:col-span-2 space-y-4">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-4 border-l-2 border-blue-500 pl-2">
                  ...
                  {UMMISCO_SITES.map((site) => (
                    <div key={site.id} className="rounded-xl border border-slate-900 bg-slate-900/10 p-5 space-y-2">
                      <div className="flex justify-between items-start">
                        ...
                        <span className="text-[12px] text-slate-500">{site.location.split(" (")[0]}</span>
                      </div>
                      ...
                      <a
                        ...
                        className="inline-flex items-center gap-1 text-[12px] text-blue-400 hover:underline font-semibold"
                      >
                        Consulter le site
                      </a>
                    </div>
                  ...
                  {INTEGRATED_APPLICATIONS.map((app) => (
                    <div key={app.id} className="rounded-lg border border-slate-900/80 bg-slate-950 p-4 space-y-2">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-[11px] bg-slate-900 border border-slate-800 text-slate-500 px-1 py-0.5 rounded uppercase">
                          {app.type}
                        </span>
                      </div>
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 text-[12px] text-blue-400 hover:text-blue-300 font-semibold"
                      >
                        Ouvrir le portail
                      </a>
                    </div>
        ...
        <section id="demandes-acl" className="py-24 px-4 sm:px-6 lg:px-8 border-t border-slate-900 bg-slate-900/10">
          <div className="mx-auto max-w-4xl">
            <div className="mb-10 text-center">
              ...
              <p className="mt-4 max-w-xl mx-auto text-slate-400 text-base leading-relaxed">
              ...
              {aclSubmitted ? (
                <div className="text-center py-8 space-y-4 animate-fadeIn">
                  ...
                  <button
                    ...
                    className="text-[13px] text-blue-400 hover:text-blue-300 font-semibold underline block mx-auto pt-2"
                  >
                ...
                <form onSubmit={handleAclSubmit} className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-1.5">
                      <label className="text-[12px] font-bold text-slate-500 uppercase tracking-wider">Nom complet</label>
                      <input
                        type="text"
                        required
                        value={aclName}
                        ...
                        className="w-full rounded-lg border border-slate-800 bg-slate-900/5 dark:bg-slate-900/50 text-sm text-slate-200 px-3.5 py-2.5 focus:outline-none focus:border-blue-500/50"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[12px] font-bold text-slate-500 uppercase tracking-wider">Adresse email</label>
                      <input
                        type="email"
                        required
                        value={aclEmail}
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[12px] font-bold text-slate-500 uppercase tracking-wider">Ressource demandée</label>
                    <select
                      value={aclDatasetId}
                      onChange={(e) => setAclDatasetId(e.target.value)}
                      className="w-full rounded-lg border border-slate-800 bg-slate-900/5 dark:bg-slate-900/50 text-sm text-slate-200 px-3 py-2.5 focus:outline-none focus:border-blue-500/50"
                    >
                      {DATASETS.map((d) => (
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[12px] font-bold text-slate-500 uppercase tracking-wider">Motifs de la demande & Projet de Recherche</label>
                    <textarea
                      required
                      rows={3}
                      value={aclReason}
                  ...
                  <button
                    type="submit"
                    className="w-full py-3 px-4 rounded-lg bg-blue-600 text-sm font-semibold uppercase tracking-wider text-white hover:bg-blue-700 active:scale-95 transition-all shadow-md"
                  >
              ...
              >
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="p-4 rounded-lg bg-slate-950 border border-slate-900 text-sm text-slate-300 leading-relaxed font-mono select-all">
                  ...
                  <div className="space-y-2">
                    ...
                    <pre className="p-4 rounded-lg bg-slate-950 border border-slate-900 text-[13px] text-slate-300 leading-relaxed font-mono overflow-x-auto select-all max-h-40">
              ...
              <div>
                ...
                {contactDone ? (
                  <div className="rounded-xl border border-green-900/30 bg-green-500/5 p-8 text-center space-y-3">
                    ...
                    <p className="text-base text-green-400 font-semibold">Message envoyé avec succès.</p>
                    <p className="text-sm text-slate-400">Nous vous répondrons sous 48h.</p>
                  </div>
                ) : (
                  <form onSubmit={handleContact} className="space-y-4">
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        <label className="text-[12px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Nom complet</label>
                        <input type="text" required value={contactNom} onChange={(e) => setContactNom(e.target.value)} placeholder="Prénom Nom" className="w-full rounded-lg border border-slate-800 bg-slate-900/5 dark:bg-slate-900/50 text-sm text-slate-200 px-3.5 py-2.5 focus:outline-none focus:border-blue-500/50" />
                      </div>
                      <div>
                        <label className="text-[12px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Email</label>
                        <input type="email" required value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} placeholder="email@example.com" className="w-full rounded-lg border border-slate-800 bg-slate-900/5 dark:bg-slate-900/50 text-sm text-slate-200 px-3.5 py-2.5 focus:outline-none focus:border-blue-500/50" />
                      </div>
                    </div>
                    <div>
                      <label className="text-[12px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Sujet</label>
                      <input type="text" value={contactSujet} onChange={(e) => setContactSujet(e.target.value)} placeholder="Collaboration, information, candidature..." className="w-full rounded-lg border border-slate-800 bg-slate-900/5 dark:bg-slate-900/50 text-sm text-slate-200 px-3.5 py-2.5 focus:outline-none focus:border-blue-500/50" />
                    </div>
                    <div>
                      <label className="text-[12px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Message</label>
                      <textarea required rows={4} value={contactMessage} onChange={(e) => setContactMessage(e.target.value)} placeholder="Votre message..." className="w-full rounded-lg border border-slate-800 bg-slate-900/5 dark:bg-slate-900/50 text-sm text-slate-200 px-3.5 py-2.5 focus:outline-none focus:border-blue-500/50" />
                    </div>
                    <button type="submit" disabled={contactLoading} className="inline-flex items-center gap-2 rounded-lg bg-ummisco-blue px-5 py-3 text-sm font-semibold uppercase tracking-wider text-white hover:bg-ummisco-blue/90 disabled:opacity-60 active:scale-95 transition-all">
              ...
              <div className="space-y-8">
                <div className="rounded-2xl border border-slate-900 bg-slate-900/10 p-8">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-base font-bold text-white">Restez informé(e)</h3>
                  </div>
                  <p className="text-sm text-slate-400 mb-5 leading-relaxed">Recevez nos dernières publications, événements et actualités du laboratoire UMMISCO.</p>
                  {nlDone ? (
                    ...
                    <form onSubmit={handleNewsletter} className="flex gap-2">
                      <input type="email" required value={nlEmail} onChange={(e) => setNlEmail(e.target.value)} placeholder="votre@email.com" className="flex-1 rounded-lg border border-slate-800 bg-slate-950/60 text-sm text-slate-200 px-3 py-2.5 focus:outline-none focus:border-blue-500/50" />
                      <button type="submit" disabled={nlLoading} className="rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60 active:scale-95 transition-all">
                        {nlLoading ? "..." : "S'abonner"}
                      </button>
                    </form>
                  )}
                ...
                <div className="rounded-2xl border border-slate-900 bg-gradient-to-br from-blue-950/40 to-slate-900/20 p-8">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-base font-bold text-white">Assistant IA UMMISCO</h3>
                  </div>
                  <p className="text-sm text-slate-400 mb-5 leading-relaxed">Posez vos questions sur nos recherches, publications, datasets et événements. Notre assistant IA vous répond en temps réel, 24h/24.</p>
